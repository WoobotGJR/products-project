...
class ProductControllerTest {
    ...
    @Test
    @DisplayName("Exception NoSuchElementException should be added to model as errors")
    void handleNoSuchElementException_ReturnsErrors404() {
        // given
        var exception = new NoSuchElementException("Product has not been found");
        ...
        assertEquals("Product has not been found", model.getAttribute("error"));
    ...
    void loadProduct_ProductExists_ReturnsNotEmptyMono() {
        ...
        this.controller.loadProduct(1))
                // then
                .expectNext(new Product(1, "Title", "Details"))
                .expectComplete()
                .verify();
    ...
    @Test
    void loadProduct_ProductDoesNotExist_ReturnsMonoWithNotSuchElementException() {
        // given
        doReturn(Mono.empty())
                .when(this.productsRestClient)
                .findProduct(1);
    ...
    void loadProduct_ProductDoesNotExist_ReturnsMonoWithNoSuchElementException() {
        // given
        doReturn(Mono.empty()).when(this.productsRestClient).findProduct(1);
    ...
    void getProductPage_ReturnsProductPage() {
        ...
        verifyNoMoreInteractions(this.productsRestClient, this.favouriteProductsClient);
    ...
    void addProductToFavourites_RequestIsValid_RedirectsToProductPage() {
        ...
        verifyNoMoreInteractions(this.favouriteProductsClient);
        verifyNoInteractions(this.productReviewsClient, this.productsRestClient);
    }
    ...
    void removeProductFromFavourites_RedirectsToProductPage() {
        ...
        // when
        StepVerifier.create(this.controller.deleteProductFromFavourites(
                        Mono.just(new Product(1, "Товар №1", "Описание товара №1"))))
                // then
                .expectNext("redirect:/customer/products/1")
                .verifyComplete();
        ...
        verifyNoMoreInteractions(this.favouriteProductsClient);
        verifyNoInteractions(this.productsRestClient, this.productReviewsClient);
    }
    ...
    void createReview_RequestIsValid_RedirectsToProductPage() {
        ...
        verifyNoInteractions(this.productsRestClient, this.favouriteProductsClient);
    }
